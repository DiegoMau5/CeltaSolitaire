window.addEventListener("load", goPlay)
...
function goPlay() {
    ...
    document.addEventListener("click", colocar);
    ...
    document.getElementById('situar').addEventListener("click", goPlay)
...
function colocar(event) {
    if (event.target.className == 'gap') {
        document.getElementById(event.target.id).className = 'bola';
    } else if(event.target.className == 'bola'){
      document.getElementById(event.target.id).className = 'gap';
    }
    else {
        if (event.target.id == "jugar") {
            document.addEventListener("click", jugar);
            document.removeEventListener("click", colocar);
...
function comprobacionMovimientos() {
    ...
    if (bolaConMovimiento.length < 1) {
        ...
        } else { document.getElementById('etiqueta_puntuacion').style.color = "red";}
    }
